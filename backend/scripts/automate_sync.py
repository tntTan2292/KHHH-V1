...
def run_sync():
    ...
    try:
        ...
        # Cho phép đồng bộ tất cả các ngày còn thiếu trong log
        folders_to_sync = [f for f in remote_folders if f not in synced_folders]
        
        if not folders_to_sync:
            logger.info("Khong co du lieu moi. He thong da cap nhat nhat.")
            # Kiem tra xem folder ngay hom nay da co tren server chua
            if expected_str not in remote_folders:
                attempt.status = 'MISSING_DATA'
                attempt.error_details = f"Folder {expected_str} chua xuat hien tren SFTP server."
        ...
        downloaded_files = []
        pending_logs = []  # [FIX] Giu danh sach log de cap nhat AFTER import thanh cong
        for f_name in folders_to_sync:
            ...
            # [FIX] Chi luu metadata, CHUA danh dau COMPLETED
            # Trang thai COMPLETED se duoc cap nhat SAU khi import thanh cong
            ...
            log.status = "DOWNLOADED"  # [FIX] Chi danh dau DOWNLOADED, chua COMPLETED
            db.commit()
            pending_logs.append((f_name, log))
        ...
        if downloaded_files:
            ...
            # [FIX] Chi sau khi import thanh cong moi danh dau COMPLETED
            for f_name, log in pending_logs:
...
if __name__ == "__main__":
    ...
    # Force UTF-8 for Windows console (only if stdout exists)

...
# Trỏ thẳng tới kho MASTER dùng chung cho cả V1 và V2
LOCAL_DOWNLOAD_DIR = r"d:\Antigravity - Project\DATA_MASTER\batch_files"
...
class SFTPManager:
    ...
    def run_command(commands):
        ...
        try:
            ...
            # Ẩn hoàn toàn cửa sổ CMD khi chạy WinSCP
            creationflags = 0
            if os.name == 'nt':
            ...
            # Kiểm tra xem có dấu hiệu login thành công không
            if "Authenticated." in result.stdout or "Session started." in result.stdout:
                return result.stdout
                
            # Nếu không có dấu hiệu thành công mà có lỗi ở stderr
            if result.returncode != 0:
                err_msg = result.stderr or result.stdout or "Kết nối SFTP thất bại"
    ...
    def list_folders():
        ...
        output = SFTPManager.run_command([f"open {SFTP_SESSION} -hostkey=*", "ls /"])
        folders = []

...
:: Sử dụng pythonw (Windowless) để không hiện cửa sổ CMD khi chạy
set "PYTHON_EXE=%PROJECT_DIR%backend\venv\Scripts\pythonw.exe"

:: Xóa task cũ nếu có và tạo mới vơi cấu hình chạy ẩn hoàn toàn
schtasks /delete /tn "VnPost_Daily_Sync" /f >nul 2>&1
:: Không dùng /it để đảm bảo chạy ở chế độ Background
schtasks /create /tn "VnPost_Daily_Sync" /tr "\"%PYTHON_EXE%\" \"%SCRIPT_PATH%\"" /sc daily /st 08:00 /ri 60 /du 12:00 /f /rl highest
...
if %errorLevel% == 0 (
    ...
    echo Sếp có thể kiểm tra trong "Task Scheduler" của Windows.
    echo ===================================================
) else (
    echo.

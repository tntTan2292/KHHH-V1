...
def debug_excel_all():
    ...
    print("Listing Row 2 to 20, columns 0-100 values to see what's there...")
    for i in range(2, 50):
        row = df.iloc[i]
        vals = [str(v).strip() for v in row.values]
        # Print if anything matches KHHH loosely
        matches = [v for v in vals if "KH" in v]
        if matches:
            print(f"Row {i} matches: {matches}")
            # Identify which column index it was
